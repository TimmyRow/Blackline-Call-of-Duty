...
function fixture(context='playing'){
 const state={context,vehicle:false,active:true,keys:new Set(),move:[0,0],look:[0,0],fire:false,aim:false,pauses:0,disconnects:0,wakes:0,clicks:0,stops:0};
 ...
 const api=createGamepad({enabled:()=>state.active,context:()=>state.context,vehicle:()=>state.vehicle,key(k,v){v?state.keys.add(k):state.keys.delete(k);},move(x,z){state.move=[x,z];},look(x,y){state.look=[x,y];},fire(v){state.fire=v;},aim(v){state.aim=v;},active(){state.active=true;},disconnect(){state.disconnects++;state.context='paused';},pause(){state.pauses++;state.context=state.context==='playing'?'paused':'playing';},map(){state.context='map';},stop(){state.stops++;},back(){state.context='playing';},panel:()=>({querySelectorAll:()=>[button]}),wake(){state.wakes++;}});
...
test('pause clears movement ADS fire and boost; held inputs require neutral before resuming',()=>{
 const f=fixture();f.state.vehicle=true;f.pad.axes=[.8,0,.5,0];f.press(6);f.press(7);f.press(10);f.poll();assert(f.state.fire&&f.state.aim);assert(f.state.keys.has('ShiftLeft'));f.press(8);f.poll();assert.equal(f.state.context,'paused');assert.deepEqual(f.state.move,[0,0]);assert(!f.state.fire&&!f.state.aim);assert.equal(f.state.keys.size,0);
...
test('disconnect releases controls exactly once and reconnect can resume',()=>{
 const f=fixture();f.press(7);f.press(10);f.poll();f.disconnect();f.poll();f.poll();assert.equal(f.state.disconnects,1);assert(!f.state.fire);assert.equal(f.state.keys.size,0);f.press(7,0);f.press(10,0);f.reconnect();f.poll();f.press(8);f.poll();assert.equal(f.state.context,'playing');assert(f.api.connected);
});
...
test('D-pad down toggles crouch on foot; A jumps; vehicles use hold-to-descend',()=>{
 const f=fixture();f.poll();f.press(13);f.poll();f.press(13,0);f.poll();assert(f.state.keys.has('KeyC'));
 f.press(0);f.poll();assert(!f.state.keys.has('KeyC'));assert(f.state.keys.has('Space'));f.press(0,0);f.poll();
 f.state.vehicle=true;f.press(13);f.poll();assert(f.state.keys.has('KeyC'));f.press(13,0);f.poll();assert(!f.state.keys.has('KeyC'));
});
...
test('click sprint continues after release, stops on neutral or aim, and resets at menus',()=>{
 ...
 f.pad.axes[1]=0;f.poll();assert(!f.state.keys.has('ShiftLeft'));f.press(13);f.poll();assert(f.state.keys.has('KeyC'));f.api.reset();assert.equal(f.state.keys.size,0);
});
...
test('requested layout maps Menu to map, View to pause, X to interact, and B to cancel/stop',()=>{
 const f=fixture();f.pad.axes[1]=-1;f.press(2);f.press(7);f.poll();assert(f.state.keys.has('KeyE'));assert(!f.state.keys.has('KeyR'));assert(f.state.fire);
 f.press(1);f.poll();assert.equal(f.state.stops,1);assert.equal(f.state.keys.size,0);assert.deepEqual(f.state.move,[0,0]);assert(!f.state.fire);f.poll();assert.equal(f.state.stops,2);
 for(let i=0;i<17;i++)f.press(i,0);f.pad.axes[1]=0;f.poll();f.press(9);f.poll();assert.equal(f.state.context,'map');assert.equal(f.state.pauses,0);
 f.press(9,0);f.poll();f.state.context='playing';f.poll();f.press(8);f.poll();assert.equal(f.state.context,'paused');assert.equal(f.state.pauses,1);
});
