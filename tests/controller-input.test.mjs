...
function fixture(context='playing'){
 const state={context,active:true,keys:new Set(),move:[0,0],look:[0,0],fire:false,aim:false,pauses:0,disconnects:0,wakes:0,clicks:0};
 ...
 const api=createGamepad({enabled:()=>state.active,context:()=>state.context,key(k,v){v?state.keys.add(k):state.keys.delete(k);},move(x,z){state.move=[x,z];},look(x,y){state.look=[x,y];},fire(v){state.fire=v;},aim(v){state.aim=v;},active(){state.active=true;},disconnect(){state.disconnects++;state.context='paused';},pause(){state.pauses++;state.context=state.context==='playing'?'paused':'playing';},missions(){state.context='journal';},back(){state.context='playing';},panel:()=>({querySelectorAll:()=>[button]}),wake(){state.wakes++;}});
...
test('pause clears movement ADS fire and boost; held inputs require neutral before resuming',()=>{
 const f=fixture();f.pad.axes=[.8,0,.5,0];f.press(6);f.press(7);f.press(10);f.poll();assert(f.state.fire&&f.state.aim);assert(f.state.keys.has('ShiftLeft'));f.press(9);f.poll();assert.equal(f.state.context,'paused');assert.deepEqual(f.state.move,[0,0]);assert(!f.state.fire&&!f.state.aim);assert.equal(f.state.keys.size,0);
