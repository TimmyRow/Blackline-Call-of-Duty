...
test('saved expedition time reproduces climate and a complete day wraps',()=>{
 ...
 assert.equal(climateAt(0).weather,'rain');
 assert.equal(climateAt(0).daylight,0);
});
test('weather transitions are continuous and include clear skies and storms',()=>{
 const states=new Set();let previous=climateAt(0);
 for(let t=1;t<WEATHER_DURATION*8;t++){const next=climateAt(t);states.add(next.weather);assert.ok(Math.abs(next.rain-previous.rain)<.02);assert.ok(next.rain>=0&&next.rain<=1);previous=next;}
