...
for(const quality of ['high','low']){
 await page.selectOption('#quality',quality,{force:true});await page.evaluate(()=>{window.blacklineQA.start();window.blacklineQA.teleport(0,8);});await page.waitForTimeout(1500);await page.keyboard.down('w');await page.mouse.move(720,450);await page.mouse.down();
 ...
 await page.keyboard.up('w');await page.mouse.up();console.log(JSON.stringify({quality,scenario:'harbour combat, forward movement and automatic fire',timing,snapshot:await page.evaluate(()=>window.blackline.snapshot())}));
}
await browser.close();
