...
try {
 await p.goto('http://localhost:5180');await p.waitForFunction(()=>window.blacklineQA,null,{timeout:90000});await p.selectOption('#quality','low',{force:true});await p.evaluate(()=>window.blacklineQA.start());await tap(14);await tap(5);
 ...
 await tap(9);assert(await p.locator('.field-map').isVisible());const before=await p.locator('.fm-chart canvas').evaluate(c=>c.toDataURL());
