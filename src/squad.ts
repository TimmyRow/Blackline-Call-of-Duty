...
export function createSquad(scene:THREE.Scene,world:RAPIER.World){
 ...
 function safeGround(x:number,z:number,expectedFeet:number){
  const hit=world.castRayAndGetNormal(new RAPIER.Ray({x,y:expectedFeet+1.2,z},{x:0,y:-1,z:0}),4.5,true,fixedFlags,undefined,undefined,undefined,squadSurface);
  if(!hit||hit.normal.y<.65)return null;
  const y=expectedFeet+1.2-hit.timeOfImpact;let occupied=false;
  world.intersectionsWithShape({x,y:y+.97,z},{x:0,y:0,z:0,w:1},probeShape,()=>{occupied=true;return false;},fixedFlags,undefined,undefined,undefined,squadSurface);
 ...
 return {
   ...
   for(let i=0;i<members.length;i++){
    ...
    for(const radius of [2.6,1.5,3.8,.85]){
     for(let n=0;n<12;n++){
      ...
      if(world.castRay(new RAPIER.Ray(start,delta),.98,true,fixedFlags,undefined,undefined,undefined,squadSurface))continue;
      placeAt=p;break;
     }
     if(placeAt)break;
    }
  ...
  step(dt:number,time:number,player:THREE.Vector3,yaw:number,targets:SquadTarget[],blocked:(a:THREE.Vector3,b:THREE.Vector3)=>boolean,onFire:(target:SquadTarget,from:THREE.Vector3,to:THREE.Vector3,info?:SquadShot)=>void,onEvent?:(message:string)=>void){
   ...
   for(let i=0;i<members.length;i++){
    ...
    if(order==='attack'&&focus){const toward=player.clone().sub(focus.position);toward.y=0;if(toward.lengthSq()<.01)toward.z=1;toward.normalize();destination=player.clone().addScaledVector(toward,2+i).add(new THREE.Vector3(offset,0,0));}
    // Probe real nearby cover, never invent a destination through a wall or off a deck.
    if(target&&m.health<72&&order!=='hold'){
     if(time>=m.coverUntil){m.coverUntil=time+3;m.cover=null;
      for(const angle of [0,Math.PI/2,Math.PI,-Math.PI/2]){
       const p=safeGround(m.position.x+Math.cos(angle)*2.3,m.position.z+Math.sin(angle)*2.3,m.position.y);
       if(p&&!blocked(eye,p.clone().add(up))&&blocked(target.position.clone().add(targetHeight),p.clone().add(new THREE.Vector3(0,.8,0)))){m.cover=p;break;}
      }
     }
     if(m.cover)destination.copy(m.cover);
    }else m.cover=null;
    ...
    const holdToFire=!!target&&!m.cover&&order!=='attack'&&playerDistance<11&&dist<8,wantsMove=dist>1.6&&!holdToFire;
    m.status=reviving?'REVIVING':m.cover?'TAKING COVER':target?'SUPPORTING':order==='attack'?'ADVANCING':order==='hold'?'HOLDING':dist>15?'CATCHING UP':'FOLLOWING';
    ...
    if(wantsMove){
     ...
     for(const turn of [0,.55,-.55,1.05,-1.05,1.57,-1.57,2.25,-2.25]){
      ...
      for(const height of [.45,1.25]){const hit=world.castRay(new RAPIER.Ray({x:m.position.x,y:m.position.y+height,z:m.position.z},{x:ux,y:0,z:uz}),probe,true,fixedFlags,undefined,undefined,undefined,squadSurface);if(hit)clearance=Math.min(clearance,Math.max(0,hit.timeOfImpact-.36));}
      // Refuse a step with no nearby supporting floor, including the edge of a ship deck.
      const floor=world.castRay(new RAPIER.Ray({x:m.position.x+ux*.8,y:m.position.y+.6,z:m.position.z+uz*.8},{x:0,y:-1,z:0}),2.4,true,fixedFlags,undefined,undefined,undefined,squadSurface);
     ...
     if(m.canAdvance){
      ...
      const safeStep=world.castRay(new RAPIER.Ray({x:m.position.x+ux*ahead,y:m.position.y+.6,z:m.position.z+uz*ahead},{x:0,y:-1,z:0}),2,true,fixedFlags,undefined,undefined,undefined,squadSurface);
    ...
    if(target&&target.health>0&&m.cooldown<=0&&!reviving){
     const to=target.position.clone().add(targetHeight);
     if(!blocked(eye,to)){
      const shot=supportShot(i,m.shotsFired,order==='attack'),damage=shot.damage,healthBefore=target.health;m.firing=.14;m.shotsFired++;
      if(!shot.hit)to.add(new THREE.Vector3(i?1.9:-1.9,.6,0));
      onFire(target,eye,to,{name:m.name,role:m.role,damage,hit:shot.hit});m.damageDealt+=Math.min(Math.max(0,healthBefore-target.health),healthBefore);
      m.cooldown=shot.cooldown;
     }else{m.target=null;m.cooldown=.12;}
    }
    if(!target)m.health=Math.min(100,m.health+dt*1.5);
   }
  },
  render(dt:number,time:number,player:THREE.Vector3){for(const m of members){if(embarked){m.visual.group.visible=false;continue;}const p=m.body.translation();m.position.set(p.x,p.y-.95,p.z);m.visual.group.position.copy(m.position);m.visual.group.visible=m.position.distanceTo(player)<180;m.visual.update(dt,{time,moving:m.moving,firing:m.firing>0,dead:m.health<=0,crouching:!!m.cover,alert:!!m.target});}},
 };
}
