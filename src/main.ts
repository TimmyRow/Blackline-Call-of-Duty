...
function mobileHint(text:string){if(gamepadMode)return text.replace(/hold USE/gi,'HOLD RB').replace(/HOLD E/g,'HOLD RB').replace(/F boards/g,'Right stick click boards').replace(/\bF\b/g,'R3').replace(/\bE\b/g,'RB').replace(/\bQ\b/g,'D-pad ← / Focus').replace(/\bH\b/g,'D-pad →').replace(/\bTab\b|\bTAB\b/g,'D-pad ↓').replace(/\bSpace\b|\bSPACE\b/g,'A').replace(/\bC\b/g,'B').replace(/\bShift\b|\bSHIFT\b/g,'L3').replace(/\bR ·/g,'X ·');return touchMode?text.replace(/F boards/g,'Tap BOARD for').replace(/F beside the hull/g,'Tap BOARD beside the hull').replace(/HOLD E/g,'HOLD USE').replace(/\bF\b/g,'BOARD / EXIT').replace(/\bE\b/g,'USE').replace(/\bQ\b/g,'MORE → FOCUS').replace(/\bH\b/g,'MORE → FIND SHIP').replace(/\bTab\b|\bTAB\b/g,'MAP').replace(/\bSpace\b|\bSPACE\b/g,'RISE').replace(/\bC\b/g,'DESCEND').replace(/\bSHIFT\b/g,'BOOST'):text;}
...
function toggleFlight(){
 ...
 else if(flight.board(camera.position)){saveGame();finishRecovery('complete');squad.embark();collider.setEnabled(false);const pose=flight.cameraPose();state.yaw=pose.yaw;state.pitch=pose.pitch;toast(gamepadMode?'RIGHT STICK · LOOK   LEFT STICK · THRUST   A / B · ALTITUDE':touchMode?'SWIPE TO LOOK · RISE TO TAKE OFF':'MOUSE LOOK · WASD THRUST · SPACE UP · C DOWN',4);subtitle('Rook: Squad aboard. Set your heading. The carrier is a safe place to refuel and regroup.',5);}
...
function updateHud(){
 ...
 $('journal-hint').textContent=gamepadMode?'VIEW · MISSIONS':'I · MISSIONS';$('controller-hint').hidden=!gamepadMode;$('controller-hint').textContent=flying?'LS · Thrust   A / B · Rise / descend   L3 · Toggle boost   R3 · Exit':marine.piloting?'LS · Throttle / steer   RT · Fire   R3 · Exit   VIEW · Journal':'LS · Move / click to sprint   A · Jump   B · Crouch   RB · Use   VIEW · Journal';$('utility').hidden=flying;$('utility').firstChild!.textContent=gamepadMode?'LB · FRAG × ':touchMode?'FRAG × ':'G · FRAG × ';if(gamepadMode){$('ship-hint').textContent='D-pad → · FIND KESTREL';$('squad-cluster').querySelector('small')!.textContent='D-pad ← · SQUAD ORDERS';}
