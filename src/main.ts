...
import {mainQuest,advanceStory,meetPerson,acceptPersonQuest,speakMain,personTarget,personAvailable} from './story-quests.mjs';
...
function updateHud(){
 ...
 const resident=!flying&&!boating&&!opening.active?questPeople.nearby(camera.position,campaign,blocked):null;if(resident)prompt='HOLD E · TALK TO '+resident.name.toUpperCase();
 ...
 state.upload=near?(campaign.progress[near.id]??0):0;$('prompt').hidden=!prompt;$('prompt-text').textContent=mobileHint(prompt);$('objective-detail').textContent=mobileHint($('objective-detail').textContent||'');$('terminal-progress').style.width=`${campaign.onboarding?.progress?campaign.onboarding.progress/2*100:resident?resident.progress/.45*100:boardingPrompt?boardingPrompt.progress*100:encounterPrompt?encounterPrompt.progress*100:near?state.upload/(near.faction==='friendly'?1.2:2.5)*100:(revivePrompt?.progress??0)*100}%`;$('damage').style.opacity=String(Math.max((flying?flight.health:state.health)<30?.25:0,1-(state.time-state.lastDamage)*1.5)*.7);
}
