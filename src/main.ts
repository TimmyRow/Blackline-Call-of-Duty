import {HANGAR,SHIP_BERTH,SHIP_PRICE,PURCHASE_ROUTE,usesShipPurchase,ownsShip,shipOffer,buyFirstShip} from './ship-purchase.mjs';
...
import {prepareShipPurchaseSave,beginCrashfall,crashfallStatus,crashfallEnemies,stepCrashfall,CRASHFALL_RALLY,recoveryBrief} from './opening-mission.mjs';
...
$('mission-title').textContent='CRASHFALL / AN ORISON EXPEDITION';$('eyebrow').textContent='PLANET EXPEDITION · GROUND / OCEAN / SPACE';$('mission-copy').textContent='A quiet descent becomes a fight to survive. Survive the crash, help the settlements and earn your first ship in Port Astra City. Choose your own path across a contested world.';
...
$('settings').insertAdjacentHTML('beforeend','<div class="controller-guide"><h3>XBOX CONTROLLER</h3><p>Connect your controller and press a button. An initial click or tap may be needed to enable sound.</p><p>Sticks · Move / look<br>RT · Fire &nbsp; LT · Aim<br>A · Start / jump / rise &nbsp; B · Cancel / stop<br>X · Hold to interact &nbsp; Y · Switch weapon<br>LB · Grenade &nbsp; RB · Reload<br>Left stick click · Toggle sprint / boost while moving<br>Right stick click · Board / exit<br>Menu (☰) · Map &nbsp; View (▣) · Pause<br>D-pad ↑ Scan · ↓ Crouch / descend · ← Journal / squad · → Ship<br>Menus: A select · B return · LB/RB tabs<br>Map: Right stick pan · LB/RB zoom</p><label>Controller look speed<input id="pad-sensitivity" type="range" min="0.4" max="2" step="0.1" value="1"></label><label><input id="pad-invert" type="checkbox"> Invert controller vertical look</label></div>');
...
function shipSite(){if(!ownsShip(campaign))return {...HANGAR,description:shipOffer(campaign).description};return {id:'kestrel',name:'YOUR SHIP / KESTREL',x:flight.position.x,z:flight.position.z,elevation:flight.position.y-1,kind:'ship',faction:'friendly',radius:8,description:touchMode?'Your dropship. Tap More → Find ship to track this beacon.':'Your personal dropship. H tracks this live beacon from anywhere.'};}
function recoveryTarget(){const stage=campaign.onboarding?.stage;if(stage==='cell')return {id:'recovery-cell',name:'EMERGENCY POWER CELL',x:RECOVERY_CELL.x,z:RECOVERY_CELL.z,elevation:heightAt(RECOVERY_CELL.x,RECOVERY_CELL.z),kind:'recovery',faction:'friendly',radius:5};const rally=crashfallStatus(campaign);if(rally)return rally.target;if((stage==='ship'||stage==='launch')&&!usesShipPurchase(campaign))return shipSite();return null;}
function finishOpening(){if(!opening.active)return;const exit=opening.finish();document.body.classList.remove('arriving');env.sync(exit);placePlayer(exit);safePosition.copy(exit);state.yaw=0;state.pitch=0;state.health=100;state.lastDamage=state.time;campaign.onboarding.introSeen=true;beginCrashfall(campaign);world.step();squad.disembark(exit,0);syncEnemies();journal(campaign,'Crashfall','The transport is lost. Recover the emergency cell and get the survivors to Pathfinder. A ship of our own will have to wait.',state.time);subtitle('Rook: We made it. The cyan case by the wreck has a power cell. Get it to the survivor rally. Then we head for Pathfinder on foot.',8);saveGame();}
function finishRecovery(event:string){const q=campaign.onboarding;if(!q||q.stage==='complete')return;if(event==='complete'){q.stage='complete';campaign.salvage+=50;journal(campaign,'Your horizon','Kestrel is yours. +50 salvage. Follow a distress signal, raid a camp or find the friendly carrier.',state.time);toast('KESTREL RECOVERED · +50 SALVAGE · THE HORIZON IS YOURS',5);}else if(event==='cell'){if(crashfallStatus(campaign)){const r=CRASHFALL_RALLY;squad.disembark(new THREE.Vector3(r.x,r.elevation+1.7,r.z),0);squad.toggle();}journal(campaign,'Emergency cell recovered',usesShipPurchase(campaign)?'Bring the cell to the survivor rally, then meet Mara in Pathfinder.':'Follow the Kestrel beacon. Hold E beside its hull to reconnect flight systems.',state.time);subtitle(crashfallStatus(campaign)?'Vale: Cell is intact. Regroup at the survivor rally. Pirates saw the crash — we need to hold them off before crossing to Pathfinder.':'Vale: Cell is intact. Kestrel is north, past the road signs. I’ll cover you.',7);}else{flight.repair();subtitle('Rook: Systems green. F boards the ship. Space takes us up. Where we go next is your call.',6);}saveGame();}
let purchaseHold=0;
function syncShipOwnership(park=false){flight.setAvailable(ownsShip(campaign));if(!ownsShip(campaign)&&park){flight.restoreAt(SHIP_BERTH);parkedShipPosition.copy(flight.position);}}
function stepShipPurchase(dt:number){
 if(ownsShip(campaign)||flight.piloting||marine.piloting||siteDistance(camera.position,HANGAR)>4||!keys.has('KeyE')){purchaseHold=0;return;}
 purchaseHold+=dt;if(purchaseHold<1.5)return;purchaseHold=0;keys.delete('KeyE');
 const result=buyFirstShip(campaign,camera.position);if(!result.ok){toast(result.reason??'Unable to purchase ship',5);return;}
 syncShipOwnership();campaign.activeMission='main';trackStory();saveGame();
 journal(campaign,'A ship of your own','Purchased Kestrel at Port Astra Hangar 03 for 300 salvage. Your squad can now travel by air and reach other worlds.',state.time);
 subtitle('Hangar control: Payment received. Kestrel is yours. Board the ship in the open-sky berth. The launch corridor is clear.',7);toast('KESTREL PURCHASED · BOARD IN HANGAR 03',5);saveGame();
}
function stepRecovery(dt:number){
 if(crashfallStatus(campaign)){
  ...
  if(event==='defended'&&usesShipPurchase(campaign)){campaign.activeMission='main';trackStory();journal(campaign,'Survivors safe','+60 salvage. Meet Mara in Pathfinder. The road to Port Astra and your first ship starts here.',state.time);subtitle('Rook: Survivors clear. Mara has shelter in Pathfinder. We travel on foot for now — Port Astra sells ships, once we have the money and clearance.',8);toast('SURVIVORS SAFE · +60 SALVAGE · MEET MARA',5);}
  else if(event==='defended'){journal(campaign,'Survivors clear','The crash survivors reached cover. +60 salvage. Follow Kestrel’s beacon to Pathfinder Landing.',state.time);subtitle('Rook: Survivors clear! Kestrel is north through the road signs. Take the cell to the hull and we can get airborne.',7);toast('RALLY SECURED · +60 SALVAGE',4);}
...
function mapSnapshot(){return {controller:gamepadMode,aboard:flight.piloting,target:resolvedTarget(),planetName:getPlanetAt(camera.position.x,camera.position.z).name,position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},yaw:state.yaw,completed:[...campaign.completed,...campaign.encountersCompleted],signals:[...QUEST_PEOPLE.filter(p=>campaign.metPeople?.includes(p.id)&&personAvailable(p,campaign)).map(personTarget),...(encounters?.sites()??[]),...(flight?[shipSite()]:[])],boat:marine?{x:marine.position.x,z:marine.position.z}:undefined,discovered:campaign.discovered,tracked:campaign.tracked,patrols:enemies.map(e=>({x:e.position.x,z:e.position.z,alive:e.active&&e.health>0})),intel:true,ship:flight&&ownsShip(campaign)?{x:flight.position.x,z:flight.position.z}:undefined,allies:squad?.members.map(m=>({x:m.position.x,z:m.position.z,alive:m.health>0}))??[]};}
function openMap(){if(state.mode!=='playing'&&state.mode!=='paused')return;setMenu('paused');mapOpen=true;$('menu').hidden=true;fieldMap.open(mapSnapshot());}
function closeMap(){if(!mapOpen)return;mapOpen=false;fieldMap.close();setMenu('playing');void capturePointer();}

function toast(text:string,seconds=2){$('toast').textContent=mobileHint(text);toastUntil=total+seconds;$('toast').style.opacity='1';}
...
function reset(withOpening=false){
 ...
 if(withOpening)campaign.completed.push(PURCHASE_ROUTE);syncShipOwnership(true);purchaseHold=0;
...
function handleKey(code:string,repeat=false){
 ...
 keys.add(code);
 if(code==='KeyH'&&!repeat){if(!ownsShip(campaign)){campaign.activeMission='main';trackStory();toast(shipOffer(campaign).description,6);}else{campaign.activeMission='free';campaign.tracked='kestrel';toast('KESTREL BEACON TRACKED · FOLLOW THE CYAN COMPASS MARK');}return;}
...
window.addEventListener('keydown',e=>{
 if(controllerOwnsInput()){e.preventDefault();return;}
 // Browser modifiers are never gameplay inputs. Ctrl+W/Ctrl+Shift+W can close Edge
 // before a page receives the shortcut, so descent must use a plain letter key.
 if(e.ctrlKey||e.metaKey||e.altKey){
  if(state.mode==='playing'){
   e.preventDefault();for(const code of keyboardKeys)if(!padKeys.has(code))keys.delete(code);keyboardKeys.clear();
   if(!e.repeat&&['ControlLeft','ControlRight'].includes(e.code))toast('USE C TO DESCEND / CROUCH · CTRL + W CLOSES THE BROWSER TAB',6);
  }
  return;
 }
 if(state.mode==='playing'&&['Space','Tab'].includes(e.code))e.preventDefault();useKeyboard();keyboardKeys.add(e.code);handleKey(e.code,e.repeat);
});
...
window.addEventListener('mousedown',e=>{if(state.mode!=='playing'||e.target!==renderer.domElement||controllerOwnsInput())return;useKeyboard();if(e.button===0){firing=true;dragging=true;}if(e.button===2)aiming=true;});
...
window.addEventListener('mousemove',e=>{
 if(state.mode!=='playing'||opening?.active||gamepadMode||padSettings.mode==='controller'){flightMouse=null;return;}
...
function mobileHint(text:string){if(gamepadMode)return text.replace(/hold USE/gi,'HOLD X').replace(/HOLD E/g,'HOLD X').replace(/F boards/g,'Right stick click boards').replace(/\bF\b/g,'R3').replace(/\bE\b/g,'X').replace(/\bQ\b/g,'D-pad ← / Focus').replace(/\bH\b/g,'D-pad →').replace(/\bTab\b|\bTAB\b/g,'☰').replace(/\bSpace\b|\bSPACE\b/g,'A').replace(/\bC\b/g,'D-pad ↓').replace(/\bShift\b|\bSHIFT\b/g,'L3').replace(/\bR ·/g,'RB ·');return touchMode?text.replace(/F boards/g,'Tap BOARD for').replace(/F beside the hull/g,'Tap BOARD beside the hull').replace(/HOLD E/g,'HOLD USE').replace(/\bF\b/g,'BOARD / EXIT').replace(/\bE\b/g,'USE').replace(/\bQ\b/g,'MORE → FOCUS').replace(/\bH\b/g,'MORE → FIND SHIP').replace(/\bTab\b|\bTAB\b/g,'MAP').replace(/\bSpace\b|\bSPACE\b/g,'RISE').replace(/\bC\b/g,'DESCEND').replace(/\bSHIFT\b/g,'BOOST'):text;}
...
function controllerOwnsInput(){if(padSettings.mode==='keyboard')return false;if(ready&&!padSettings.testing&&document.hasFocus())gamepad?.poll(0,performance.now());return gamepadMode||padSettings.mode==='controller';}
...
gamepad=createGamepad({settings:()=>padSettings.value,enabled:()=>gamepadMode&&ready&&padSettings.mode!=='keyboard'&&!padSettings.testing,vehicle:()=>!!(flight?.piloting||marine?.piloting),panMap:(x,z,dt)=>fieldMap.pan(x,z,dt),context:()=>journalOpen?'journal':mapOpen?'map':!$('settings').hidden?'settings':state.mode==='playing'&&opening?.active?'intro':state.mode,
 active(){if(padSettings.mode==='keyboard'||padSettings.testing)return;padSettings.lockController();if(!gamepadMode){touch?.reset();touchMove.x=touchMove.z=0;for(const code of keyboardKeys)if(!padKeys.has(code))keys.delete(code);keyboardKeys.clear();firing=aiming=dragging=false;}gamepadMode=true;document.body.classList.add('controller-active');$('controller-hint').hidden=state.mode!=='playing';},wake(){if(ready&&(!sound.ctx||sound.ctx.state==='suspended'))sound.start();},
 ...
 fire(v){padFire=v;},aim(v){padAim=v;},disconnect(){gamepadMode=false;document.body.classList.remove('controller-active');if(state.mode==='playing')setMenu('paused');toast('CONTROLLER DISCONNECTED · RECONNECT OR SELECT KEYBOARD IN SETTINGS',5);},
 pause(){if(state.mode==='menu'||state.mode==='dead'||state.mode==='won'){if(!$('settings').hidden)$('settings-close').click();return;}if(journalOpen||mapOpen||!$('settings').hidden||state.mode==='playing')setMenu('paused');else if(state.mode==='paused')void deploy();},map(){if(mapOpen)closeMap();else openMap();},stop(){firing=aiming=false;keys.clear();keyboardKeys.clear();padKeys.clear();touch?.reset();touchMove.x=touchMove.z=0;if(flight?.piloting)flight.velocity.set(0,0,0);if(marine?.piloting)marine.stop();},back(){if(opening?.active&&state.mode==='playing')finishOpening();else controllerBack();},panel:activePanel});

// A mapped desktop click must not double-activate a menu after the real pad button.
window.addEventListener('click',e=>{if(e.isTrusted&&!padSettings.testing&&controllerOwnsInput()&&performance.now()-(gamepad?.lastActivity??-Infinity)<250){e.preventDefault();e.stopImmediatePropagation();}},true);
window.addEventListener('blackline-input-mode',()=>{gamepad?.reset();keys.clear();keyboardKeys.clear();padKeys.clear();firing=aiming=dragging=false;gamepadMode=padSettings.mode==='controller';document.body.classList.toggle('controller-active',gamepadMode);});
$('controls-button').insertAdjacentHTML('afterend','<button class="secondary" id="controller-setup-button">CONTROLLER SETUP</button>');
$('controller-setup-button').onclick=()=>{$('controls-button').click();padSettings.startTest();$('settings').scrollTop=0;};
...
function toggleFlight(){
 ...
 if(!flight.piloting&&marine.board(camera.position)){squad.embark();collider.setEnabled(false);toast(gamepadMode?'LEFT STICK · THROTTLE / STEER · R3 EXIT WHEN STOPPED':'W/S THROTTLE · A/D RUDDER · F EXIT WHEN STOPPED',5);return;}
 ...
 else if(!ownsShip(campaign)){toast(shipOffer(campaign).description,6);return;}
 else if(['cell','ship'].includes(campaign.onboarding?.stage)){toast('KESTREL NEEDS ITS POWER CELL · FOLLOW THE RECOVERY BEACON');return;}
 else if(flight.board(camera.position)){saveGame();finishRecovery('complete');squad.embark();collider.setEnabled(false);const pose=flight.cameraPose();state.yaw=pose.yaw;state.pitch=pose.pitch;toast(gamepadMode?'RIGHT STICK · LOOK   LEFT STICK · THRUST   A / D-PAD ↓ · ALTITUDE':touchMode?'SWIPE TO LOOK · RISE TO TAKE OFF':'MOUSE LOOK · WASD THRUST · SPACE UP · C DOWN',4);subtitle('Rook: Squad aboard. Set your heading. The carrier is a safe place to refuel and regroup.',5);}
...
function fixedStep(dt:number){
 state.time+=dt;state.shield=rechargeShield(state,dt,state.time,campaign.upgrades?.shieldCell||0);state.energy=rechargeEnergy(state.energy,dt,state.time-state.lastShot);stepRecovery(dt);stepShipPurchase(dt);encounters.update(dt,state.time,camera.position);if(state.time>streamAt){streamAt=state.time+.6;syncEnemies();}
 ...
 if(flying){const before=flight.position.clone(),healthBefore=flight.health;flight.step(dt,{keys:movementKeys(),move:analogMove(),yaw:state.yaw,pitch:state.pitch});if(flight.health<healthBefore)state.lastDamage=state.time;camera.position.copy(flight.cameraPose().position);campaign.distanceFlown+=before.distanceTo(flight.position);if(isFiring())fireCannons();if(flight.health<=0){hurt(1000);return;}}
  ...
  let p=body.translation();const wantsCrouch=keys.has('KeyC');
 ...
 settleContracts();if(state.time-lastSave>(flying?5:20))saveGame();
...
function updateHud(){
 ...
 const localSite=nearbySites.find(s=>siteDistance(camera.position,s)<s.radius),tracked=resolvedTarget(),site=tracked??(!flying&&!campaign.tracked&&ownsShip(campaign)?shipSite():null);
 $('objective-phase').textContent=`${getPlanetAt(camera.position.x,camera.position.z).name.toUpperCase()} / ${campaign.discovered.length} DISCOVERED · ${campaign.raids} RAIDS`;$('objective-name').textContent=localSite?localSite.name:flying?'The horizon is yours':'Explore the open frontier';$('objective-detail').textContent=localSite?(localSite.faction==='pirate'?`${guardCount(localSite.id)} pirates here · Q focuses squad fire`:localSite.description):flying?'Space / C · Altitude   Shift · Boost   Tab · Atlas':'Raid camps · Discover relics · F boards Kestrel';$('telemetry').firstChild!.textContent=currentRegion.toUpperCase();$('network-strength').textContent=String(campaign.salvage);
 ...
 $('journal-hint').textContent=gamepadMode?'D-pad ← · JOURNAL':'I · MISSIONS';$('controller-hint').hidden=!gamepadMode;$('controller-hint').textContent=flying?'LS · Thrust   A · Rise   D-pad ↓ · Descend   B · Stop   R3 · Exit':marine.piloting?'LS · Throttle / steer   B · Stop   R3 · Exit   ☰ · Map':'LS · Move / click to sprint   A · Jump   X · Use   B · Stop   ☰ · Map';$('utility').hidden=flying;$('utility').firstChild!.textContent=gamepadMode?'LB · FRAG × ':touchMode?'FRAG × ':'G · FRAG × ';if(gamepadMode){$('ship-hint').textContent='D-pad → · FIND KESTREL';$('squad-cluster').querySelector('small')!.textContent='D-pad ← · SQUAD ORDERS';}
 const stage=campaign.onboarding?.stage;if(stage&&stage!=='complete'){const brief=recoveryBrief(campaign);$('objective-phase').textContent=usesShipPurchase(campaign)?'CRASHFALL / SURVIVE THE LANDING':'CRASHFALL / RECOVER YOUR SHIP';$('objective-name').textContent=brief.title;$('objective-detail').textContent=brief.description;}
 navigationUI.update({position:camera.position,yaw:state.yaw,tracked,ship:ownsShip(campaign)?shipSite():null,aboard:flying,flying,landed:fs.landed,speed:fs.speed,altitude:fs.altitude,visible:state.mode==='playing'});$('shield-label').hidden=flying;$('shield-track').hidden=flying;$('shield-label').textContent=`SHIELD ${Math.ceil(state.shield)} · ${gamepadMode?'Y':'X'} · ${state.weapon==='energy'?'BALLISTIC':'ENERGY'}`;$('shield-fill').style.width=`${state.shield/shieldCapacity(campaign.upgrades?.shieldCell||0)*100}%`;$('crosshair').classList.toggle('aiming',isAiming()&&!flying);
 const near=nearbySites.find(s=>siteDistance(camera.position,s)<6);const encounterPrompt=encounters.prompt(camera.position,guardCount);let prompt='';if(boating)prompt='W/S THROTTLE · A/D RUDDER · STOP THEN F TO LEAVE HELM';else if(flying)prompt=flight.landed?'F · DISEMBARK / SPACE · TAKE OFF':'SPACE ↑   C ↓   SHIFT BOOST · LAND SLOWLY TO EXIT';else if(revivePrompt)prompt=`HOLD E · REVIVE ${revivePrompt.name}`;else if(encounterPrompt)prompt=encounterPrompt.text;else if(near)prompt=interactionLabel(campaign,near,guardCount(near.id),state.time);else prompt=marine.nearbyPrompt(camera.position)??flight.nearbyPrompt(camera.position)??(state.reload>0?'RELOADING':state.ammo===0?'R · RELOAD':'');
 ...
 if(!ownsShip(campaign)&&!flying&&!boating&&siteDistance(camera.position,HANGAR)<4){$('region-arrival').style.opacity='0';prompt=shipOffer(campaign).available?`HOLD E · BUY KESTREL / ${SHIP_PRICE} SALVAGE`:shipOffer(campaign).description;}
 state.upload=near?(campaign.progress[near.id]??0):0;$('prompt').hidden=!prompt;$('prompt-text').textContent=mobileHint(prompt);$('objective-detail').textContent=mobileHint($('objective-detail').textContent||'');$('terminal-progress').style.width=`${purchaseHold?purchaseHold/1.5*100:campaign.onboarding?.progress?campaign.onboarding.progress/(crashfallStatus(campaign)?.stage==='regroup'?.6:2)*100:resident?resident.progress/.45*100:boardingPrompt?boardingPrompt.progress*100:encounterPrompt?encounterPrompt.progress*100:near?state.upload/(near.faction==='friendly'?1.2:2.5)*100:(revivePrompt?.progress??0)*100}%`;$('damage').style.opacity=String(Math.max((flying?flight.health:state.health)<30?.25:0,1-(state.time-state.lastDamage)*1.5)*.7);
}
...
function animate(now:number){
 requestAnimationFrame(animate);if(graphicsLost||document.hidden){last=now;accumulator=0;frames=0;frameSum=0;return;}const realElapsed=(now-last)/1000,elapsed=Math.min(.05,realElapsed);last=now;total+=elapsed;padSettings.poll(now);if(document.hasFocus()&&!padSettings.testing)gamepad?.poll(elapsed,now);else gamepad?.reset();frames++;frameSum+=realElapsed;
...
function journalData(){const fs=flight.snapshot(),planet=getPlanetAt(camera.position.x,camera.position.z);return {campaign,mainQuest:mainQuest(campaign),person:conversationData(),controller:gamepadMode,recovery:recoveryTarget()?{title:usesShipPurchase(campaign)?'Crashfall / Protect the survivors':'Crashfall / Recover Kestrel',description:recoveryBrief(campaign).description}:null,missions:missionBoard(campaign,missionSites(),camera.position),atBase:atBase(),saveMessage,planetName:planet.name,scanner:{cooldown:Math.max(0,scanReady-state.time),range:1100+350*(campaign.upgrades?.scanner||0)},war:regionalWar(campaign,nearbySites),boarding:boardingStatus(campaign),destinations:PLANETS.map(p=>{const reason=jumpPermission({flying:flight.piloting,altitude:fs.altitude,speed:fs.speed,health:fs.health,energy:fs.energy,cooldown:jumpReady-state.time,current:planet.id,destination:p.id});return {id:p.id,name:p.name,description:p.description,available:!reason,reason:reason??undefined};})};}
...
function startMission(id:string){if(id==='main'){campaign.activeMission='main';trackStory();saveGame();closeJournal();return;}const mission=missionBoard(campaign,missionSites(),camera.position).find(m=>m.id===id);if(!mission||mission.status==='complete')return;acceptContract(campaign,id);campaign.activeMission=id;if(mission.target){knownSites.set(mission.target.id,mission.target as Site);campaign.tracked=mission.target.id;}else campaign.tracked=id==='aces'?'kestrel':null;saveGame();closeJournal();toast(recoveryTarget()?'MISSION ACCEPTED · FINISH CRASHFALL FIRST':mission.title.toUpperCase()+' · '+mission.instruction,5);}
...
async function boot(){
 try{
  ...
  const diagnostics={snapshot:()=>({mode:state.mode,opening:opening.snapshot(),crashfall:crashfallStatus(campaign),shipPurchase:{owned:ownsShip(campaign),offer:shipOffer(campaign),hangar:HANGAR},touchMode,input:{controller:gamepadMode,inputMode:padSettings.mode,testing:padSettings.testing,move:analogMove(),firing:isFiring(),aiming:isAiming(),held:[...keys],yaw:state.yaw,pitch:state.pitch,fov:camera.fov},pixelRatio:renderer.getPixelRatio(),weapon:state.weapon,energy:state.energy,shield:state.shield,planet:getPlanetAt(camera.position.x,camera.position.z).id,boarding:boarding.snapshot(campaign),story:mainQuest(campaign),talking:talkingId,people:QUEST_PEOPLE.filter(p=>personAvailable(p,campaign)).map(p=>({...personTarget(p),personId:p.id})),scanCooldown:Math.max(0,scanReady-state.time),graphics:{antialias:renderer.getContext().getContextAttributes()?.antialias,lost:graphicsLost,recoveries:graphicsRecoveries,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures,enemyPool:enemies.length},health:state.health,ammo:state.ammo,reserve:state.reserve,grenades:state.grenades,kills:state.kills,stage:state.stage,upload:state.upload,time:state.time,position:camera.position.toArray(),region:currentRegion,campaign:JSON.parse(JSON.stringify(campaign)),flight:flight.snapshot(),marine:marine.snapshot(),climate:env.climate(),encounters:encounters.snapshot(),saveMessage,resolutionScale,airPirates:spacePirates.snapshot(),world:env.stats(),sites:nearbySites,guards:enemies.filter(e=>e.active).length,squad:squad.snapshot(),enemyPositions:enemies.map(e=>({id:e.id,site:e.site,active:e.active,position:e.position.toArray(),health:e.health,shield:e.shield,role:e.role,staggerUntil:e.staggerUntil})),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,renderCpuMs:lastRenderMs})};
...
document.querySelector('.control-grid')?.insertAdjacentHTML('beforeend','<span><b>I</b> Journal / upgrades / contracts</span><span><b>F</b> Board / exit ship or boat</span><span><b>Q / J</b> Squad focus / supplies</span><span><b>FLIGHT</b> WASD thrust · Space / C altitude</span><span><b>SHIFT / FIRE</b> Boost / cannons</span>');
