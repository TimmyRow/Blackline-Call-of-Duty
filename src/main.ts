...
import {REGION_START,REGION_SITES,SEA_LEVEL,getWorldSites,getLandingPads,heightAt,regionName} from './region-layout.mjs';
...
let flight:ReturnType<typeof createFlight>;let spacePirates:ReturnType<typeof createSpacePirates>;
...
type Enemy={id:string,site:string,active:boolean,home:THREE.Vector3,visual:ReturnType<typeof createEnemy>,position:THREE.Vector3,health:number,timer:number,alert:number,seed:number,body:RAPIER.RigidBody,collider:RAPIER.Collider,controller:RAPIER.KinematicCharacterController,lastSeen:THREE.Vector3,flashUntil:number,moving:boolean};
const enemies:Enemy[]=[];
...
function mapSnapshot(){return {position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},yaw:state.yaw,completed:campaign.completed,discovered:campaign.discovered,tracked:campaign.tracked,patrols:enemies.map(e=>({x:e.position.x,z:e.position.z,alive:e.active&&e.health>0})),intel:true,ship:flight?{x:flight.position.x,z:flight.position.z}:undefined,allies:squad?.members.map(m=>({x:m.position.x,z:m.position.z,alive:m.health>0}))??[]};}
...
function reset(){
 for(const id of campaign.completed)env.setSiteComplete(id,false);campaign=createAdventure();enemyHealth.clear();for(const e of enemies){e.active=false;e.collider.setEnabled(false);e.visual.group.visible=false;}streamAt=-1;spacePirates.reset();flight.reset();waterExposure=0;currentRegion='South Landing';regionToastUntil=0;
...
window.addEventListener('keydown',e=>{
 ...
 if(e.code==='KeyQ'&&!e.repeat&&!flight.piloting){focusTarget();return;}
 if(e.code==='KeyJ'&&!e.repeat&&!flight.piloting){if(squad.supply(camera.position)){state.reserve=Math.min(360,state.reserve+90);state.grenades=Math.min(5,state.grenades+1);toast('ROOK · AMMUNITION DELIVERED');}else toast('SUPPLY UNAVAILABLE · REGROUP WITH SQUAD');return;}
 if(flight.piloting)return;
...
function damageEnemy(e:Enemy,damage:number,at:THREE.Vector3,source?:string){if(!e.active||e.health<=0)return;e.health-=damage;enemyHealth.set(e.id,e.health);e.alert=8;sparks(at,5);if(!source){sound.hit();hitUntil=total+.13;$('hitmarker').style.opacity='1';}if(e.health<=0){state.kills++;e.collider.setEnabled(false);if(source){squad.registerKill(source);toast(`${source} · PIRATE DOWN`,1);}else toast('PIRATE NEUTRALIZED',1);}}
function fire(){
...
function syncEnemies(){
 env.sync(camera.position);nearbySites=getWorldSites(camera.position.x,camera.position.z,1200);for(const site of nearbySites)knownSites.set(site.id,site);
 const desired:{id:string,site:string,x:number,y:number,z:number}[]=[];
 for(const site of nearbySites){if(site.faction!=='pirate'||campaign.completed.includes(site.id)||Math.abs(site.elevation-camera.position.y)>230||Math.hypot(site.x-camera.position.x,site.z-camera.position.z)>650)continue;
  const count=site.kind==='pirate-ship'?8:6;for(let i=0;i<count;i++){const angle=i/count*Math.PI*2,r=12+(i%2)*7,x=site.x+Math.cos(angle)*r,z=site.z+Math.sin(angle)*r;desired.push({id:`${site.id}:${i}`,site:site.id,x,y:site.elevation+.2,z});}
 }
 ...
 for(const d of desired.slice(0,48)){let e=enemies.find(e=>e.id===d.id);if(e?.active)continue;e??=enemies.find(e=>!e.active);
  if(!e){const visual=createEnemy(scene),rb=world.createRigidBody(RAPIER.RigidBodyDesc.kinematicPositionBased()),ec=world.createCollider(RAPIER.ColliderDesc.capsule(.6,.28),rb),ctrl=world.createCharacterController(.025);ctrl.enableSnapToGround(.65);ctrl.enableAutostep(.45,.2,true);e={id:'',site:'',active:false,home:new THREE.Vector3(),visual,position:new THREE.Vector3(),health:100,timer:2,alert:0,seed:enemies.length*2.7,body:rb,collider:ec,controller:ctrl,lastSeen:new THREE.Vector3(),flashUntil:0,moving:false};visual.group.traverse(o=>{if(o instanceof THREE.Mesh)o.castShadow=false;});enemies.push(e);}
  Object.assign(e,{id:d.id,site:d.site,active:true,health:enemyHealth.get(d.id)??100,timer:1.8,alert:0});e.position.set(d.x,d.y,d.z);e.home.copy(e.position);e.lastSeen.copy(e.position);e.body.setTranslation({x:d.x,y:d.y+.95,z:d.z},true);e.body.setNextKinematicTranslation({x:d.x,y:d.y+.95,z:d.z});e.collider.setEnabled(e.health>0);e.visual.group.position.copy(e.position);e.visual.group.visible=true;
 }
}
...
function fireCannons(){const shot=flight.fire();if(!shot)return;sound.shot();const obstruction=world.castRay(new RAPIER.Ray(shot.origin,shot.direction),900,true,RAPIER.QueryFilterFlags.ONLY_FIXED);const air=spacePirates.hit(shot.origin,shot.direction,obstruction?.timeOfImpact??900);let end=shot.origin.clone().addScaledVector(shot.direction,900);if(air){end=air.position;sparks(end,12);if(air.killed){campaign.airKills++;campaign.salvage+=80;toast('PIRATE INTERCEPTOR DESTROYED · +80 SALVAGE');}}
 else{const hit=obstruction;if(hit){end=shot.origin.clone().addScaledVector(shot.direction,hit.timeOfImpact);sparks(end,12);}for(const e of enemies){if(!e.active||e.health<=0)continue;const delta=e.position.clone().add(new THREE.Vector3(0,1,0)).sub(shot.origin),along=delta.dot(shot.direction);if(along>0&&along<shot.origin.distanceTo(end)+3&&delta.addScaledVector(shot.direction,-along).length()<3)damageEnemy(e,60,e.position.clone().add(new THREE.Vector3(0,1,0)));}}
 tracer(shot.origin,end,0x91e5ff);
}
function fixedStep(dt:number){
 state.time+=dt;if(state.time>streamAt){streamAt=state.time+.6;env.sync(flight.piloting?flight.position:camera.position);syncEnemies();}
 const flying=flight.piloting;let crouching=false,sprinting=false;
 ...
 for(const e of enemies){
  if(!e.active||e.health<=0)continue;e.timer-=dt;e.alert-=dt;const eye=e.position.clone().add(new THREE.Vector3(0,1.55,0));
  const threats=[...(!flying?[{position:camera.position,index:-1,range:60*(crouching?.65:1)}]:[]),...(!squad.embarked?squad.members.flatMap((m,i)=>m.health>0?[{position:m.position.clone().add(new THREE.Vector3(0,1.5,0)),index:i,range:65}]:[]):[])];
  const threat=threats.sort((a,b)=>a.position.distanceTo(eye)-b.position.distanceTo(eye)).find(t=>t.position.distanceTo(eye)<t.range&&!blocked(eye,t.position));if(threat){e.alert=6;e.lastSeen.copy(threat.position);}
  const patrol=e.home.clone().add(new THREE.Vector3(Math.sin(state.time*.22+e.seed)*4,0,Math.cos(state.time*.22+e.seed)*4)),goal=e.alert>0?e.lastSeen:patrol,dx=goal.x-e.position.x,dz=goal.z-e.position.z,distance=Math.hypot(dx,dz),len=distance||1,speed=e.alert>0?(distance>18?2.6:distance<8?-.5:0):distance>1?1.1:0,drift=e.alert>0?Math.sin(state.time*.8+e.seed)*.5:0;
  ...
  if(threat&&e.timer<=0){e.timer=1.2+Math.random()*1.2;e.flashUntil=state.time+.12;tracer(eye,threat.position.clone().add(new THREE.Vector3((Math.random()-.5)*1.8,-.15,0)),0xff8755);if(eye.distanceTo(camera.position)<70)sound.shot(true);if(Math.random()<(threat.index<0?(sprinting?.2:crouching?.25:.4):.55)){if(threat.index<0)hurt(4+Math.random()*3);else squad.hurt(threat.index,4+Math.random()*4);}}
 }
 squad.step(dt,state.time,camera.position,state.yaw,enemies.filter(e=>e.active),blocked,(target,from,to,info)=>{tracer(from,to,0x7dd9ff);if(from.distanceTo(camera.position)<70)sound.shot(true);damageEnemy(target as Enemy,info?.damage??24,to,info?.name??'SQUAD');},message=>subtitle(message,3));
 spacePirates.step(dt,camera.position,flying,(a,b)=>tracer(a,b,0xff785a),n=>flight.hurt(n));
 world.timestep=dt;world.step();
 if(!flying){const bp=body.translation();campaign.distanceWalked+=Math.hypot(bp.x-camera.position.x,bp.z-camera.position.z);camera.position.x=bp.x;camera.position.z=bp.z;camera.position.y=THREE.MathUtils.lerp(camera.position.y,bp.y+(crouching?.48:.7),.35);if(camera.position.y< -150)hurt(100);}
 ...
 if(!flying){const event=stepAdventure(campaign,nearbySites,camera.position,keys.has('KeyE'),dt,guardCount,state.time);if(event){if(event.type==='discover')toast(`DISCOVERED · ${event.site.name.toUpperCase()}`,3);else if(event.type==='resupply'){state.health=100;state.reserve=300;state.grenades=4;flight.repair();for(const m of squad.members)if(m.health>0)m.health=100;toast('SQUAD RESUPPLIED · KESTREL REPAIRED');}else{env.setSiteComplete(event.site.id,true);state.reserve=Math.min(360,state.reserve+60);toast(`${event.site.name.toUpperCase()} · CARGO RECOVERED`,3);subtitle('Rook: Cargo secured. There are more signals beyond this coast. Your call where we go next.',5);}}revivePrompt=squad.reviveNear(camera.position,keys.has('KeyE'),dt);}
...
function updateHud(){
 const fs=flight.snapshot(),flying=flight.piloting;$('ammo').textContent=String(Math.round(flying?fs.energy:state.ammo)).padStart(2,'0');$('reserve').textContent=flying?'% ENERGY':`/ ${state.reserve}`;$('grenades').textContent=String(state.grenades);$('health-value').textContent=String(Math.ceil(flying?flight.health:state.health));$('health-fill').style.width=`${flying?flight.health:state.health}%`;$('health-cluster').classList.toggle('danger',state.health<35);$('weapon-name').textContent=flying?'KESTREL / PULSE CANNONS':'MK18 MOD 1 / AUTO';$('stance').textContent=flying?`${Math.round(fs.speed)} M/S · ${Math.round(camera.position.y)} M ALT`:waterExposure?'SWIMMING':crouched?'CROUCHED':keys.has('ShiftLeft')?'SPRINTING':'STANDING';
 ...
 const ss=squad.snapshot();$('squad-order').textContent=`SQUAD / ${squad.embarked?'ABOARD KESTREL':squad.order.toUpperCase()}`;$('squad-members').textContent=ss.members.map(m=>`${m.name} ${m.down?'DOWN':m.status} · ${m.kills} K`).join(' / ');$('region-arrival').style.opacity=total<regionToastUntil?'1':'0';
 ...
 const near=nearbySites.find(s=>siteDistance(camera.position,s)<6);let prompt='';if(flying)prompt=flight.landed?'F · DISEMBARK / SPACE · TAKE OFF':'SPACE ↑   CTRL ↓   SHIFT BOOST · LAND SLOWLY TO EXIT';else if(revivePrompt)prompt=`HOLD E · REVIVE ${revivePrompt.name}`;else if(near)prompt=interactionLabel(campaign,near,guardCount(near.id),state.time);else prompt=flight.nearbyPrompt(camera.position)??(state.reload>0?'RELOADING':state.ammo===0?'R · RELOAD':'');
 state.upload=near?(campaign.progress[near.id]??0):0;$('prompt').hidden=!prompt;$('prompt-text').textContent=prompt;$('terminal-progress').style.width=`${near?state.upload/(near.faction==='friendly'?1.2:2.5)*100:(revivePrompt?.progress??0)*100}%`;$('damage').style.opacity=String(Math.max(state.health<30?.25:0,1-(state.time-state.lastDamage)*1.5)*.7);
}
...
function animate(now:number){
 ...
 if(state.mode==='playing'){
  ...
  if(flight.piloting){const pose=flight.cameraPose();camera.position.copy(pose.position);camera.rotation.set(pose.pitch,pose.yaw,0,'YXZ');}else camera.rotation.set(state.pitch+state.recoil*.2,state.yaw,0,'YXZ');state.recoil*=Math.exp(-elapsed*15);
 ...
 weapon.group.visible=!flight.piloting;flight.render(elapsed,state.time);
 ...
 for(const e of enemies){e.visual.group.visible=e.active&&e.position.distanceTo(camera.position)<280;if(e.visual.group.visible)e.visual.update(state.mode==='playing'?elapsed:0,{time:state.time+e.seed,moving:e.health>0&&e.moving,firing:e.health>0&&state.time<e.flashUntil,dead:e.health<=0});}
 ...
 env.update(elapsed,total,camera.position);
...
async function boot(){
 try{
  ...
  squad=createSquad(scene,world);flight=createFlight(scene,world);spacePirates=createSpacePirates(scene);syncEnemies();
  world.step();scene.updateMatrixWorld(true);await renderer.compileAsync(scene,camera);ready=true;$<HTMLButtonElement>('deploy').disabled=false;$('deploy-label').textContent='BEGIN EXPEDITION';$('loading').textContent='F · Board Kestrel   /   Tab · Planet atlas   /   Q · Squad focus';
...
document.querySelector('.control-grid')?.insertAdjacentHTML('beforeend','<span><b>F</b> Board / exit ship (landed)</span><span><b>Q / J</b> Squad focus / supplies</span><span><b>FLIGHT</b> WASD thrust · Space / Ctrl altitude</span><span><b>SHIFT / FIRE</b> Boost / cannons</span>');
