...
import {scanSignals,regionalWar,jumpPermission,boardingStatus} from './expedition-rules.mjs';
...
function setMenu(mode:Mode){
 ...
 touch?.reset();if(mode!=='playing')sound.setFlight(false);
...
function fire(){
 ...
 if(state.weapon==='energy'){if(state.energy<WEAPONS.energy.cost)return;state.energy-=WEAPONS.energy.cost;}else{if(state.ammo===0){reload();return;}state.ammo--;}state.lastShot=state.time;state.shots++;state.recoil=Math.min(.09,state.recoil+gun.recoil*(1-.12*(campaign.upgrades?.handling||0)));state.pitch=Math.min(1.4,state.pitch+(aiming?.0025:.006));weapon.flash();sound.shot(false,state.weapon);
 scene.updateMatrixWorld(true);ray.setFromCamera(new THREE.Vector2((Math.random()-.5)*(aiming?.0008:.008),(Math.random()-.5)*(aiming?.0008:.008)),camera);ray.far=gun.range;
...
function fixedStep(dt:number){
 ...
 if(boarding.step(dt,camera.position,keys.has('KeyE'),guardCount('corsair'),campaign)){journal(campaign,'Prisoner rescued','The Corsair prisoner is safe. +120 salvage. Recover the cargo to claim this ship.',state.time);toast('PRISONER RESCUED · +120 SALVAGE',4);saveGame();}
...
function updateHud(){
 ...
 $('world-waypoint').hidden=!site;if(site){const worldTarget=new THREE.Vector3(site.x,site.elevation+3,site.z);camera.updateMatrixWorld();const behind=worldTarget.clone().applyMatrix4(camera.matrixWorldInverse).z>0;worldTarget.project(camera);$('world-waypoint').style.left=`${(behind?.5:THREE.MathUtils.clamp((worldTarget.x+1)/2,110/innerWidth,1-110/innerWidth))*100}%`;$('world-waypoint').style.top=`${(behind?.76:THREE.MathUtils.clamp((1-worldTarget.y)/2,.2,.75))*100}%`;$('world-waypoint').querySelector('span')!.textContent=behind?'↶':'◇';const nav=targetNavigation(camera.position,state.yaw,site);$('waypoint-label').textContent=`${site.name} · ${nav?.distanceLabel??''} · ${nav?.verticalLabel??''}`;}
 ...
 navigationUI.update({position:camera.position,yaw:state.yaw,tracked,ship:shipSite(),aboard:flying,flying,landed:fs.landed,speed:fs.speed,altitude:fs.altitude,visible:state.mode==='playing'});$('shield-label').hidden=flying;$('shield-track').hidden=flying;$('shield-label').textContent=`SHIELD ${Math.ceil(state.shield)} · ${state.weapon==='energy'?'X · BALLISTIC':'X · ENERGY'}`;$('shield-fill').style.width=`${state.shield/shieldCapacity(campaign.upgrades?.shieldCell||0)*100}%`;$('crosshair').classList.toggle('aiming',aiming&&!flying);
 ...
 state.upload=near?(campaign.progress[near.id]??0):0;$('prompt').hidden=!prompt;$('prompt-text').textContent=mobileHint(prompt);$('objective-detail').textContent=mobileHint($('objective-detail').textContent||'');$('terminal-progress').style.width=`${campaign.onboarding?.progress?campaign.onboarding.progress/2*100:encounterPrompt?encounterPrompt.progress*100:near?state.upload/(near.faction==='friendly'?1.2:2.5)*100:(revivePrompt?.progress??0)*100}%`;$('damage').style.opacity=String(Math.max((flying?flight.health:state.health)<30?.25:0,1-(state.time-state.lastDamage)*1.5)*.7);
}
...
async function jumpPlanet(id:string){
 ...
 try{
  ...
  jumpReady=state.time+Math.max(15,45-10*(campaign.upgrades?.reactor||0));campaign.tracked=id==='vesper'?'vesper-port':'kestrel';
...
function openJournal(){if(state.mode!=='playing')return;setMenu('paused');journalOpen=true;$('menu').hidden=true;expeditionUI.open(journalData());}
