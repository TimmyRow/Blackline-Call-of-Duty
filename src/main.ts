...
function mobileHint(text:string){if(gamepadMode)return text.replace(/HOLD E/g,'HOLD RB').replace(/F boards/g,'Right stick click boards').replace(/\bF\b/g,'R3').replace(/\bE\b/g,'RB').replace(/\bQ\b/g,'D-pad ← / Focus').replace(/\bH\b/g,'D-pad →').replace(/\bTab\b|\bTAB\b/g,'D-pad ↓').replace(/\bSpace\b|\bSPACE\b/g,'A').replace(/\bC\b/g,'B').replace(/\bShift\b|\bSHIFT\b/g,'L3').replace(/\bR ·/g,'X ·');return touchMode?text.replace(/F boards/g,'Tap BOARD for').replace(/F beside the hull/g,'Tap BOARD beside the hull').replace(/HOLD E/g,'HOLD USE').replace(/\bF\b/g,'BOARD / EXIT').replace(/\bE\b/g,'USE').replace(/\bQ\b/g,'MORE → FOCUS').replace(/\bH\b/g,'MORE → FIND SHIP').replace(/\bTab\b|\bTAB\b/g,'MAP').replace(/\bSpace\b|\bSPACE\b/g,'RISE').replace(/\bC\b/g,'DESCEND').replace(/\bSHIFT\b/g,'BOOST'):text;}
