...
import {WEAPONS,roleForId,roleStats,enemyTactic,weaponDamage,damageCombat,rechargeShield,rechargeEnergy,shieldCapacity} from './combat-rules.mjs';
...
const fieldMap=createFieldMap($('field-map-root'),{onTrack:(id,site)=>{if(site)knownSites.set(id,site);campaign.tracked=id==='corsair-objective'?'corsair':id;saveGame();closeMap();},onClose:()=>closeMap()});
...
type Enemy={id:string,site:string,role:string,shield:number,active:boolean,home:THREE.Vector3,visual:ReturnType<typeof createEnemy>,position:THREE.Vector3,health:number,timer:number,alert:number,seed:number,body:RAPIER.RigidBody,collider:RAPIER.Collider,controller:RAPIER.KinematicCharacterController,lastSeen:THREE.Vector3,flashUntil:number,moving:boolean,scan:number,threat:{position:THREE.Vector3,index:number,range:number}|null};
const enemies:Enemy[]=[];
...
function stepRecovery(dt:number){const target=recoveryTarget();if(!target)return;const event=recoveryStep(campaign,camera.position,{x:target.x,y:target.elevation+1.7,z:target.z},keys.has('KeyE'),dt,flight.piloting);if(event)finishRecovery(event);}
...
function mapSnapshot(){return {aboard:flight.piloting,target:resolvedTarget(),planetName:getPlanetAt(camera.position.x,camera.position.z).name,position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},yaw:state.yaw,completed:[...campaign.completed,...campaign.encountersCompleted],signals:[...(encounters?.sites()??[]),...(flight?[shipSite()]:[])],boat:marine?{x:marine.position.x,z:marine.position.z}:undefined,discovered:campaign.discovered,tracked:campaign.tracked,patrols:enemies.map(e=>({x:e.position.x,z:e.position.z,alive:e.active&&e.health>0})),intel:true,ship:flight?{x:flight.position.x,z:flight.position.z}:undefined,allies:squad?.members.map(m=>({x:m.position.x,z:m.position.z,alive:m.health>0}))??[]};}
...
function reset(withOpening=false){
 if(opening)opening.finish();boarding?.reset();alarms.clear();scanReady=jumpReady=0;state.weapon='ballistic';state.energy=100;state.shield=70;weapon?.setWeapon('ballistic');document.body.classList.remove('arriving');
...
gamepad=createGamepad({enabled:()=>gamepadMode,context:()=>journalOpen?'journal':mapOpen?'map':!$('settings').hidden?'settings':state.mode==='playing'&&opening?.active?'intro':state.mode,
 ...
 look(x,y,dt){if(state.mode!=='playing'||opening.active)return;const speed=2.2*+$<HTMLInputElement>('pad-sensitivity').value*(isAiming()?.35:1);state.yaw-=x*Math.abs(x)*speed*dt;state.pitch=THREE.MathUtils.clamp(state.pitch-y*Math.abs(y)*speed*dt*($<HTMLInputElement>('pad-invert').checked?-1:1),-1.45,1.45);},
 ...
 pause(){if(journalOpen||mapOpen)controllerBack();else if(state.mode==='playing')setMenu('paused');else if(state.mode==='paused')void deploy();},missions(){if(journalOpen)closeJournal();else if(state.mode==='playing'||state.mode==='paused')openJournal();},back(){if(opening?.active&&state.mode==='playing')finishOpening();else controllerBack();},panel:activePanel});
...
function damageEnemy(e:Enemy,damage:number,at:THREE.Vector3,source?:string,kind=state.weapon as string){if(!e.active||e.health<=0||damage<=0)return;e.visual.hit();const previousShield=e.shield;Object.assign(e,damageCombat(e,damage,source?'ballistic':kind));if(previousShield>0&&e.shield===0)sound.shieldBreak();enemyHealth.set(e.id,e.health);e.alert=8;sparks(at,5);if(!source){sound.hit();hitUntil=total+.13;$('hitmarker').style.opacity='1';}if(e.health<=0){state.kills++;e.collider.setEnabled(false);if(source){squad.registerKill(source);toast(`${source} · PIRATE DOWN`,1);}else toast('PIRATE NEUTRALIZED',1);}}
function fire(){
...
function syncEnemies(){
 ...
 for(const d of desired.slice(0,48)){let e=enemies.find(e=>e.id===d.id);if(e?.active){if(isEncounterId(d.site)){e.home.set(d.x,d.y,d.z);}continue;}e??=enemies.find(e=>!e.active);
  ...
  const role=roleForId(d.id),stats=roleStats(role);e.visual.setRole(role);Object.assign(e,{id:d.id,site:d.site,role,shield:stats.shield,active:true,health:enemyHealth.get(d.id)??stats.health,timer:1.8,alert:0,scan:0,threat:null});e.position.set(d.x,d.y,d.z);e.home.copy(e.position);e.lastSeen.copy(e.position);e.body.setTranslation({x:d.x,y:d.y+.95,z:d.z},true);e.body.setNextKinematicTranslation({x:d.x,y:d.y+.95,z:d.z});e.collider.setEnabled(e.health>0);e.visual.group.position.copy(e.position);e.visual.group.visible=true;
 }
}
...
function fixedStep(dt:number){
 ...
 if(campaign.onboarding?.stage==='complete'&&!campaign.safeHarbour.announced&&state.time>12){campaign.safeHarbour.announced=true;journal(campaign,'Distress / Safe Harbour','Pirates seized an allied vessel southwest of the colony. Open Missions to start Operation Safe Harbour. Approach by air, or use the aft sea stairs.',state.time);subtitle('Distress channel: This is the captured Corsair. Allied crew trapped aboard. Anyone receiving? Find Operation Safe Harbour in Missions.',8);}
...
function updateHud(){
 ...
 state.upload=near?(campaign.progress[near.id]??0):0;$('prompt').hidden=!prompt;$('prompt-text').textContent=mobileHint(prompt);$('objective-detail').textContent=mobileHint($('objective-detail').textContent||'');$('terminal-progress').style.width=`${campaign.onboarding?.progress?campaign.onboarding.progress/2*100:boardingPrompt?boardingPrompt.progress*100:encounterPrompt?encounterPrompt.progress*100:near?state.upload/(near.faction==='friendly'?1.2:2.5)*100:(revivePrompt?.progress??0)*100}%`;$('damage').style.opacity=String(Math.max((flying?flight.health:state.health)<30?.25:0,1-(state.time-state.lastDamage)*1.5)*.7);
}
...
function animate(now:number){
 ...
 touch?.update(state.mode==='playing'&&!gamepadMode,opening.active,flight.piloting?'ship':marine.piloting?'boat':'foot');opening.setPaused(state.mode!=='playing');weapon.group.visible=!flight.piloting&&!opening.active;marine.render(elapsed,state.time);flight.render(elapsed,state.time);
...
function journalData(){const fs=flight.snapshot(),planet=getPlanetAt(camera.position.x,camera.position.z);return {campaign,controller:gamepadMode,recovery:recoveryTarget()?{title:'Crashfall / Recover Kestrel',description:'Your emergency cell and ship are marked. Complete recovery before taking on the wider frontier.'}:null,missions:missionBoard(campaign,missionSites(),camera.position).sort((a,b)=>Number(b.id==='raider')-Number(a.id==='raider')),atBase:atBase(),saveMessage,planetName:planet.name,scanner:{cooldown:Math.max(0,scanReady-state.time),range:1100+350*(campaign.upgrades?.scanner||0)},war:regionalWar(campaign,nearbySites),boarding:boardingStatus(campaign),destinations:PLANETS.map(p=>{const reason=jumpPermission({flying:flight.piloting,altitude:fs.altitude,speed:fs.speed,health:fs.health,energy:fs.energy,cooldown:jumpReady-state.time,current:planet.id,destination:p.id});return {id:p.id,name:p.name,description:p.description,available:!reason,reason:reason??undefined};})};}
...
function startMission(id:string){const mission=missionBoard(campaign,missionSites(),camera.position).find(m=>m.id===id);if(!mission||mission.status==='complete')return;acceptContract(campaign,id);campaign.activeMission=id;if(mission.target){knownSites.set(mission.target.id,mission.target as Site);campaign.tracked=mission.target.id;}else campaign.tracked=id==='aces'?'kestrel':null;saveGame();closeJournal();toast(recoveryTarget()?'MISSION ACCEPTED · FINISH SHIP RECOVERY FIRST':mission.title.toUpperCase()+' · '+mission.instruction,5);}
function refreshJournal(){expeditionUI.update(journalData());}
function openJournal(section='missions'){if(!ready||!['playing','paused'].includes(state.mode))return;setMenu('paused');journalOpen=true;$('menu').hidden=true;expeditionUI.open(journalData(),section);}
function closeJournal(){if(!journalOpen)return;journalOpen=false;expeditionUI.close();setMenu('playing');void capturePointer();}
function settleContracts(){const active=CONTRACTS.find(c=>c.id===campaign.activeMission&&campaign.contracts[c.id]==='active');if(active&&!active.bearing&&campaign.tracked&&(active.kind==='discoveries'&&campaign.discovered.includes(campaign.tracked)||active.kind==='scanned'&&campaign.scanned.includes(campaign.tracked)||active.kind==='signals'&&campaign.encountersCompleted.includes(campaign.tracked))){const next=missionBoard(campaign,missionSites(),camera.position).find(m=>m.id===active.id)?.target;if(next){knownSites.set(next.id,next as Site);campaign.tracked=next.id;}else campaign.tracked=null;}for(const contract of resolveContracts(campaign)){journal(campaign,contract.title,`Contract complete · ${contract.reward} salvage`,state.time);toast(`${contract.title.toUpperCase()} · +${contract.reward} SALVAGE`,4);}}
...
async function boot(){
 try{
  ...
  const diagnostics={snapshot:()=>({mode:state.mode,opening:opening.snapshot(),touchMode,input:{controller:gamepadMode,move:analogMove(),firing:isFiring(),aiming:isAiming(),held:[...keys],yaw:state.yaw,pitch:state.pitch,fov:camera.fov},pixelRatio:renderer.getPixelRatio(),weapon:state.weapon,energy:state.energy,shield:state.shield,planet:getPlanetAt(camera.position.x,camera.position.z).id,boarding:boarding.snapshot(campaign),scanCooldown:Math.max(0,scanReady-state.time),graphics:{lost:graphicsLost,recoveries:graphicsRecoveries,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures,enemyPool:enemies.length},health:state.health,ammo:state.ammo,reserve:state.reserve,grenades:state.grenades,kills:state.kills,stage:state.stage,upload:state.upload,time:state.time,position:camera.position.toArray(),region:currentRegion,campaign:JSON.parse(JSON.stringify(campaign)),flight:flight.snapshot(),marine:marine.snapshot(),climate:env.climate(),encounters:encounters.snapshot(),saveMessage,resolutionScale,airPirates:spacePirates.snapshot(),world:env.stats(),sites:nearbySites,guards:enemies.filter(e=>e.active).length,squad:squad.snapshot(),enemyPositions:enemies.map(e=>({id:e.id,site:e.site,active:e.active,position:e.position.toArray(),health:e.health,shield:e.shield,role:e.role})),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,renderCpuMs:lastRenderMs})};
