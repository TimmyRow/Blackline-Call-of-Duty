...
function reset(withOpening=false){
 if(opening)opening.finish();document.body.classList.remove('arriving');
...
function updateHud(){
 ...
 const localSite=nearbySites.find(s=>siteDistance(camera.position,s)<s.radius),tracked=campaign.tracked==='kestrel'?shipSite():campaign.tracked?knownSites.get(campaign.tracked):null,site=recoveryTarget()??tracked??(!flying?shipSite():null);
