...
gamepad=createGamepad({settings:()=>padSettings.value,enabled:()=>gamepadMode&&ready,vehicle:()=>!!(flight?.piloting||marine?.piloting),panMap:(x,z,dt)=>fieldMap.pan(x,z,dt),context:()=>journalOpen?'journal':mapOpen?'map':!$('settings').hidden?'settings':state.mode==='playing'&&opening?.active?'intro':state.mode,
 active(){if(!gamepadMode){touch?.reset();touchMove.x=touchMove.z=0;for(const code of keyboardKeys)if(!padKeys.has(code))keys.delete(code);keyboardKeys.clear();firing=aiming=dragging=false;}gamepadMode=true;document.body.classList.add('controller-active');$('controller-hint').hidden=state.mode!=='playing';},wake(){if(ready&&(!sound.ctx||sound.ctx.state==='suspended'))sound.start();},
