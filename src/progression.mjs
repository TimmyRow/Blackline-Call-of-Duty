...
export function ensureProgression(c){c.debriefedOperations??=[];c.clues??=[];c.oceanEvent??=0;c.operations??={};c.baseLocker??=normalizeBase(null);c.upgrades??={};c.contracts??={};c.journal??=[];c.encountersCompleted??=[];c.oceanKills??=0;c.blueprints??=[];c.scanned??=[];c.rescued??=[];c.boarding??={disabled:false};c.planet??='orison';c.activeMission??='main';c.metPeople??=[];c.story??={briefed:false,finished:false,notified:''};c.safeHarbour??={announced:false,defending:false,repelled:false,rewarded:false};c.onboarding??={stage:'launch',introSeen:true,progress:0};return c;}
...
export function validateSave(raw){
 ...
 c.debriefedOperations=[...new Set(strings(v.debriefedOperations))].filter(id=>OPERATIONS.some(o=>o.id===id)&&c.contracts[id]==='complete');
