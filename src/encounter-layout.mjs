...
export function getEncounterSites(x,z,radius=950){
 ...
 for(let cx=Math.floor((x-radius)/size);cx<=Math.floor((x+radius)/size);cx++)for(let cz=Math.floor((z-radius)/size);cz<=Math.floor((z+radius)/size);cz++){
  ...
  const kinds=['wreck','relic','bunker','distress','patrol'],kind=kinds[Math.floor(worldHash(cx,cz,79)*kinds.length)];
  result.push({id:`encounter:${cx}:${cz}`,name:({wreck:'Wrecked Prospector',relic:'Ancient Resonator',bunker:'Abandoned Field Bunker',distress:'Emergency Transponder',patrol:'Pirate Recon Patrol'})[kind],x:px,z:pz,kind});
 }
