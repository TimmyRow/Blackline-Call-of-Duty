...
export function createOpening(scene:THREE.Scene,camera:THREE.Camera,world:RAPIER.World){
 ...
 overlay.querySelector('p')!.textContent=time<5?'Vale: Coast looks quiet. We’ll find a place to land.':time<10?'Rook: Missile warning! Engines are hit. Hold on!':time<13?'Vale: Main engines gone. Taking her down in the clearing.':'Rook: You still with us? Get out. Get the others to cover.';
