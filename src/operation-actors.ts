...
import {normalizeRescuePositions} from './field-operations.mjs';
...
export function createOperationActors(scene:THREE.Scene,world:RAPIER.World){
 ...
 save(c:any){if(!initialized||stage(c)!==2||c.contracts?.['op-rescue']!=='active')return;const positions=normalizeRescuePositions(survivors.map(s=>{const p=s.body.translation();return[p.x,p.y,p.z];}));if(positions)c.operations['op-rescue'].survivors=positions;},
 ready(c:any,target:any){return stage(c)!==2||(initialized&&survivors.every(s=>s.visual.group.visible&&Math.hypot(s.visual.group.position.x-target.x,s.visual.group.position.z-target.z)<11));},
 update(c:any,p:THREE.Vector3,dt:number,time:number){const n=stage(c),active=c.contracts?.['op-rescue']==='active'&&n<3&&Math.hypot(p.x+175,p.z-140)<180;
  ...
  if(active&&!initialized){const saved=n===2?normalizeRescuePositions(c.operations?.['op-rescue']?.survivors):null;for(const [i,s] of survivors.entries()){const x=saved?.[i][0]??-175+s.side*4,z=saved?.[i][2]??150,y=saved?.[i][1]??heightAt(x,z)+.95;s.body.setTranslation({x,y,z},true);s.body.setNextKinematicTranslation({x,y,z});s.visual.group.position.set(x,y-.95,z);}initialized=true;}
