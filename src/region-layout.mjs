...
export function regionName(x,z){const sites=getWorldSites(x,z,220).sort((a,b)=>Math.hypot(a.x-x,a.z-z)-Math.hypot(b.x-x,b.z-z));if(sites[0]&&Math.hypot(sites[0].x-x,sites[0].z-z)<sites[0].radius+15)return sites[0].name;return heightAt(x,z)<SEA_LEVEL?'Orison Ocean':Math.hypot(x,z)<1500?'Ash Coast Wilderness':'Uncharted Frontier';}
